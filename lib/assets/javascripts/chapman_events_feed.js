...
(function ($) {
  ...
  ChapmanEventsFeed = function (options) {
    if (!options || !options.feed_path)
      throw "ChapmanEventsFeed needs to have a url passed into the options object.";
    this.per = options.per || 2;
    this.browse_link =
      options.browse_link === undefined ? true : options.browse_link;
  ...
  ChapmanEventsFeed.prototype.eventCardDate = function (date) {
    ...
    var date_html = [
      ...
      '  <div class="left date-column"><div class="day">' +
        date_arr[0] +
        "</div></div>",
      '  <div class="right date-column"><p>' +
        date_arr[1] +
        "</p><p>" +
        date_arr[2] +
        "</p></div>",
  ...
  ChapmanEventsFeed.prototype.eventCardImg = function (event) {
    var image_div = $('<div class="featured-image"></div>').css(
      "background-image",
  ...
  ChapmanEventsFeed.prototype.programCard = function (event) {
    ...
    var $eventTitle = $("<p></p>", {
      class: "program-event__title",
      text: event.title,
    });
    var $eventDate = $("<p></p>", {
      class: "program-event__date",
      text: dates[1] + ". " + dates[0] + ", " + dates[2],
    });
    var $eventTime = $("<p></p>", {
      class: "program-event__time",
      text: event.formatted_time,
    });
    var $dateContainer = $("<div></div>", {
      class: "program-event__date-container",
    });
