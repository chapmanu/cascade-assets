        ...
        success: function(data) {
          if ( data.length ) {
            ...
            if ($('.chapman-stories-feed-widget').length > 1) {
              // do stuff if widget being used more than once
              // check if each widget instance is only using 1 feed button
                // append singular feed button to it's respective parent class of stories-feed

                //checking diff of loop running twice only on local vs staging
                for(let i = 0; i < 2; i++) {
                  console.log('running: ', i);
                }
            } else {
              if ($('.stories-feed-btn').length === 1) {
                var feedBtn = $('.stories-feed-btn').detach();
                $('.stories-feed').append(feedBtn);
              }
            }
            ...
            //   ctr++;
            //   var feedBtn = $(this).find('.stories-feed-btn');
            //   var storiesFeed = $(this).find('.stories-feed');
            //   feedBtn.detach().appendTo(storiesFeed);
            // });
            // console.log("total count: " , ctr);
