...
;(function($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    ...
    buildOutWidget: function() {
      ...
      this.$element.prepend($outerContainer);
      ...
      $.ajax({
        ...
        success: function(data) {
          if ( data.length ) {
            ...
            // Handles single feed buttons for when multple instances of this widget are being used on the same page
            if ($('.chapman-stories-feed-widget').length > 1) {
              var storiesFeedMain = $('.chapman-stories-feed-widget');

              storiesFeedMain.each(function() {
                var feedButton = $(this).find('.stories-feed-btn');

                if (feedButton.length === 1) {
                  var storiesFeed = $(this).find('.stories-feed');
                  var feedButtonGroup =  $(this).find('.stories-feed-group-btn');
                  // Script/loop runs multiple times, needs the right conditions before moving the buttons
                  if (feedButtonGroup.length > 0 && storiesFeed[0].children.length > 1) {
                    $(storiesFeed).append(feedButton);
                    feedButtonGroup.remove();
                  }
                }
              });
            } else {
              if ($('.stories-feed-btn').length === 1) {
                var feedButton = $('.stories-feed-btn').detach();
                $('.stories-feed').append(feedButton);
                $('.stories-feed-group-btn').remove();
              }
            }
          }
        },
