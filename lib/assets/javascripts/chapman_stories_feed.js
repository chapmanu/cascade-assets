      ...
      $.ajax({
        ...
        success: function(data) {
          if ( data.length ) {
            ...
            // step 1: initiate loop to account for each stories feed elements ID's
            // step 2 : save each of the instances into one separate compartment according to their ID's
            // step 3: check condition of saved stories feed elements
            var storiesFeedButtons = $('.stories-feed-btn').detach();
            // console.log('stories feed elements: ', storiesFeedButtons);
            // for (const buttons in storiesFeedButtons) {
              // console.log('each loop of buttons is: ', buttons[0]);
            ...
            if ((storiesFeedButtons).length > 1) {
              $('.chapman-stories-feed-widget').append(storiesFeedButtons);
              $('.stories-feed-btn').wrapAll("<div class='stories-feed-group-btn' />");
            } else {
              $('.stories-feed').append(storiesFeedButtons);
