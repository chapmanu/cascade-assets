  ...
  if (document.getElementById("school_FacDirSearch_withoutFT") != null) {
      ...
      populateResults = function () {
        ...
        $.getJSON(facultyFeedUrl(), function (data) {
          for (var i = 0; i < data.length; i++) {
            ...
            var result = {
              link: data[i].CascadePath
                ? "/our-faculty/" + data[i].CascadePath
                : "",
              image: v_photo,
              ...
              affiliation: (function () {
                ...
                for (var j = 0; j < data[i].Depts.length; j++) {
                  cntr = cntr + 1;
                  school = data[i].Depts[j].SchoolName
                    ? data[i].Depts[j].SchoolName
                    : "";
                  if (
                    data[i].Depts[j].DisplayDeptName != "" &&
                    data[i].Depts[j].DisplayDeptName != "Conservatory of Music"
                  ) {
                    dept = data[i].Depts[j].FacGroupName
                      ? data[i].Depts[j].DisplayDeptName +
                        ", " +
                        data[i].Depts[j].FacGroupName
                      : data[i].Depts[j].DisplayDeptName;
                  } else {
                    dept = data[i].Depts[j].FacGroupName
                      ? data[i].Depts[j].FacGroupName
                  ...
                  //
                  if (cntr == 1) {
                    prev_school = data[i].Depts[j].SchoolName
                      ? data[i].Depts[j].SchoolName
                      : "";
                    prev_dept = dept;
              ...
              specialty: (function () {
                var spcl;
                if (
                  data[i].SpecialtyGrouping != "none" &&
                  data[i].SpecialtyGrouping != ""
                ) {
          ...
          if (data.length) {
            var rangeLower =
                data[data.length - 1].CurrentPage * resultsPerPage + 1,
              rangeUpper =
                (data[data.length - 1].CurrentPage + 1) *
                data[data.length - 1].ResultsPerPage,
    ...
    function formatResult(result) {
      ...
      formattedResult =
        ...
        (result.image
          ? '<div class="profilePicture"><img width="80px" src="' +
            result.image +
            '" itemprop="image" alt="' +
            result.imageAltText +
            '"></div>'
        ...
        (result.additionalTitles
          ? '<div class="additionalTitles" itemprop="jobTitle">' +
            result.additionalTitles +
            "</div>"
          : "") +
        (result.specialty
          ? '<div class="specialty" itemprop="specialty">' +
            result.specialty +
            "</div>"
          : "") +
        (result.affiliation
          ? '<div class="affiliation" itemprop="affiliation">' +
            result.affiliation +
            "</div>"
          : "") +
        (result.email
          ? '<div class="email" itemprop="email"><a href="mailto:' +
            result.email +
            '">' +
            result.email +
            "</a></div>"
