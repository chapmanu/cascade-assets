...
(function ($, Modernizr, window, document) {
  ...
  chapman.customSelect = {
    ...
    bindUIEvents: function () {
      ...
      selectContainerSelector.on("click", ".custom-select-input", function () {
        ...
        if ($(this).hasClass(openClass)) {
          // If already open, close the options
      ...
      selectContainerSelector.each(function () {
        ...
        $(this)
          .append(
            '<div id="' +
              selectID +
              '-clone" class="custom-select-input" aria-label="Click to toggle options dropdown for ' +
              selectValue +
              ' select box"><span class="selected-value">' +
              selectValue +
              '</span></div><div class="custom-select-options" data-field-id="' +
              selectID +
              '"><ul>' +
              optionsHTML +
              "</ul></div>"
          )
          .addClass("custom-select-options-active");
      });
    },
    ...
    syncCustomSelect: function (select) {
      ...
      var selectedOptionText = $(select)
        .parent()
        .find('.custom-select-options li[data-value="' + val + '"]')
        .text();
